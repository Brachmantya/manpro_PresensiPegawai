  ...
  if (isset($_POST['action'])) {
      ...
      if ($action === 'editEmployee') {
        ...
        if ($id) { 
            ...
            $gaji_perJam = is_numeric($_POST['employee_salary']) ? $_POST['employee_salary'] : 0;
    
            // SQL untuk update
            $sql = "UPDATE Pegawai SET nama=?, nomor_hp=?, email=?, alamat=?, posisi=?, gaji_perJam=? WHERE id_pegawai=?";
            $stmt = $conn->prepare($sql);
            if ($stmt) {
                $stmt->bind_param("ssssssi", $nama, $nomor_hp, $email, $alamat, $posisi, $gaji_perJam, $id);
                if ($stmt->execute()) {
  ...
  <div id="employeeInfoModal" class="modal" style="display: none;">
    <div class="modal-content">
      ...
      <form method="POST">
        ...
        <input type="hidden" name="action" id="formAction" value="editEmployee">
...
document.querySelector('form').addEventListener('submit', function () {
    ...
    if (formAction === 'editEmployee') {
        // Pastikan tidak terjadi penambahan baris baru
        const employeeId = document.getElementById('employee_id').value;

        // Perbarui row di sisi klien sebelum submit
        const row = document.getElementById(`employee-${employeeId}`);
        if (row) {
            const cells = row.getElementsByTagName('td');
            cells[0].textContent = document.getElementById('employee_name').value;
            cells[1].textContent = document.getElementById('employee_phone').value;
            cells[2].textContent = document.getElementById('employee_email').value;
            cells[3].textContent = document.getElementById('employee_address').value;
            cells[4].textContent = document.getElementById('employee_position').value;

            const salary = document.getElementById('employee_salary').value;
            cells[5].textContent = parseInt(salary).toLocaleString('id-ID');
        }
    }
    // Form akan dikirim ke server tanpa di-interupsi
});
